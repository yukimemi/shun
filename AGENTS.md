...
Project conventions and guidelines for Codex.
...
- `apm-install` requires the
  ...
  `apm.yml`, pinned to `#main`) into `.Codex/skills/` +
...
- Pre-push hook in `.Codex/settings.json`: cargo fmt --check, cargo clippy -D warnings, npm test

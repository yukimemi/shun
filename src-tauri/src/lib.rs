...
async fn install_update(app: tauri::AppHandle) -> Result<(), String> {
    match detect_install_method() {
        ...
        // Scoop インストールも portable と同じ方式（zip 直接ダウンロード → in-place 差し替え）で
        // 更新する。PowerShell 経由で `scoop update shun` を実行する方式は、PowerShell プロセスが
        // 親プロセス終了と連動して停止してしまい確実に動作しないため廃止した。
        // Scoop のバージョン管理（manifest）は次回 `scoop update shun` 実行時に自動的に同期される。
        InstallMethod::Scoop => install_update_portable(&app).await,
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // 世代カウンタ (blur_gen) で最後の blur タスクのみ hide+refresh を実行し、
            // 連続した Focused(false) イベントによる重複処理を防ぐ。
            {
